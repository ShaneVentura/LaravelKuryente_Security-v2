    ...
    @endphp
     <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if ($hasError)
                // Remove styles when there is an error
                body.classList.remove('overflow-x-hidden', 'overflow-y-hidden');
            @endif
        });
    </script>
    ...
    <form method="POST" action="{{ route('register') }}" x-data="{ personalForm: true, addressForm: false, meterForm: false, emailForm: false }">
        ...
        <div x-show="meterForm" style="display: none">
            ...
            <div class="mt-4">
                ...
                <x-text-input id="MID" class="block mt-1 w-full mb-0" type="text" name="MID"
                    :value="old('MID')" autofocus placeholder="12345678" />
                @error('MID')
                    <span class="text-sm text-red-600 space-y-1 -mb-5">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mt-4">
                ...
                <x-text-input id="PIN" class="block mt-1 w-full mb-0" type="text" name="PIN"
                    :value="old('PIN')" autofocus placeholder="12345678" />
                @error('PIN')
                    <span class="text-sm text-red-600 space-y-1 -mb-5"">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div x-show="addressForm" style="display: none">
            ...
            <div class="mt-4" x-data="{ province: 'oldValue' }">
                ...
                @error('province')
                    <span class="text-sm text-red-600 space-y-1 -mb-3">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div id="emailForm" x-show="emailForm" style="display: none">
            ...
            <div class="mt-2">
                ...
                @error('g-recaptcha-response')
                    <span class="text-sm text-red-600 space-y-1 mt-10">{{ __('Please check the box.') }}</span>
                @enderror
            </div>
